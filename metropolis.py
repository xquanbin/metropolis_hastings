...
def metropolis_hastings(data, delta, tau, rho, steps):
    ...
    for s in range(0, steps):
        ...
        # print "n_edges:{}".format(n_edges), "post_likelihood: {}".format(current_log_post)
...
def cliques_to_jtree(cliques, nodes_num, ns):
    ...
    sparse_clique = np.zeros([cliques_num, nodes_num])  # sparse.lil_matrix((cliques_num, nodes_num))
...
if __name__ == "__main__":
    ...
    # data_ = np.loadtxt(RETURN_PATH + '/all.txt')
    data_ = np.loadtxt('./input/test_data.txt')
    ...
    G = metropolis_hastings(data_, delta_, tau_, rho_, steps_)
    end_time = time.time()
